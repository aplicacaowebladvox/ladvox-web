...
import { UrlParameterModel } from '../../../core/models/url-parameter.model';
import { dateParser, dateParserSend } from '../../shared/utils/date.util';
...
export class AuthStore {
  ...
  protected requestService: AuthRequestService;

  constructor(httpBackend: HttpBackend) {
    this.requestService = new AuthRequestService(httpBackend);
    this.requestService.apiUrl = environment.config.apiUrl;

    this.baseController = AUTH_API;
  }

  protected getUrl(action: string) {
    return `${this.baseController}/${action}`;
  }
  token(input: LoginUserModel): Observable<TokenResponseModel> {
    return this.requestService.makePost(this.getUrl('token'), {
      data: input,
      useAuth: false,
    });
  }
  checkToken(token: string | null): Observable<any> {
    return this.requestService.makePost(this.getUrl(CHECK_TOKEN), {
      data: { refreshToken: token },
      useAuth: false,
    });
  }
  refreshToken(refreshToken: AuthInputRefreshTokenInterface): Observable<TokenResponseModel> {
    return this.requestService.makePost(this.getUrl('refresh-token'), {
      data: refreshToken,
      useAuth: false,
    });
  }
