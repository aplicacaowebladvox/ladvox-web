...
function genFunc(myEval, type = TYPE_FUNC, prec = 0, left = true) {
    return {
        myEval: myEval,
        t: type,
        prec: prec,
...
function genNode(myVal, func = true, unary = true) {
    return {
        val: myVal,
        func: func,
        unary: unary,
...
function myEval(tree) {
    if (tree.func) {
        if (tree.unary) {
            return tree.val.myEval(myEval(tree.left));
        } else {
            return tree.val.myEval(myEval(tree.left), myEval(tree.right));
        }
    } else {
...
module.exports = { RPN, myEval, parse }
